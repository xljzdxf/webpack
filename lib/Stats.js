	...
	toJson(options, forToString) {
		...
		if (!showOrphanModules) {
			excludeModules.push((ident, module, type) => {
				return chunkGraph.getNumberOfModuleChunks(module) === 0 && type !== "nested";
			});
		}
		...
		const fnModule = (module, nested) => {
			...
			const obj = {
				...
				chunks: Array.from(chunkGraph.getOrderedModuleChunksIterable(module, compareChunks), chunk => chunk.id),
