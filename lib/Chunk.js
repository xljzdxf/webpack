...
class Chunk {
	...
	constructor(name) {
		...
		/** @private @type {Map<Module, ChunkGroup>} */
		this._entryModules = new Map();
	...
	/**
	 * @param {Module} module the module that should be checked
	 * @returns {boolean} if this chunk contains the entry module
	 */
	hasEntryModule(module) {
		return this._entryModules.has(module);
	}

	/**
	 * @param {Module} module the module that should be an entry
	 * @param {ChunkGroup=} chunkGroup the chunk group that need to be loaded before the module should be executed
	 * @returns {void}
	 */
	addEntryModule(module, chunkGroup) {
		this._entryModules.set(module, chunkGroup);
	}

	/**
	 * @param {Module} oldModule replaced module
	 * @param {Module} newModule replacing module
	 * @returns {void}
	 */
	replaceEntryModule(oldModule, newModule) {
		if (this._entryModules.has(oldModule)) {
			/** @type {Map<Module, ChunkGroup>} */
			const newMap = new Map();
			for (const [module, chunkGroup] of this._entryModules) {
				newMap.set(module === oldModule ? newModule : module, chunkGroup);
			}
			this._entryModules = newMap;
		}
	}
	...
	getNumberOfEntryModules() {
		return this._entryModules.size;
	}

	/**
	 * @returns {Iterable<Module>} the modules for this chunk
	 */
	get entryModulesIterable() {
		return this._entryModules.keys();
	}

	/** @typedef {[Module, ChunkGroup | undefined]} EntryModuleWithChunkGroup */

	/**
	 * @returns {Iterable<EntryModuleWithChunkGroup>} the modules for this chunk
	 */
	get entryModulesWithChunkGroupIterable() {
		return this._entryModules;
	}
	...
	updateHash(hash) {
		...
		for (const [m, chunkGroup] of this._entryModules) {
			hash.update("entry");
			hash.update(m.hash);
	...
	canBeIntegrated(otherChunk) {
		...
		if (
			this.getNumberOfEntryModules() > 0 ||
			otherChunk.getNumberOfEntryModules() > 0
		) {
			return false;
		}
