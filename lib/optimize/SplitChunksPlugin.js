...
module.exports = class SplitChunksPlugin {
		...
		if (cacheGroups && typeof cacheGroups === "object") {
			const fn = (module, context) => {
				...
				for (const key of Object.keys(cacheGroups)) {
					...
					} else if (SplitChunksPlugin.checkTest(option.test, module, context)) {
						if (results === undefined) results = [];
						results.push({
							key: key,
	 ...
	 * @param {ModuleGraph} context.moduleGraph the module graph
				...
				chunks => {
					...
					for (const module of compilation.modules) {
						...
						if (!chunkSetsInGraph.has(chunksKey)) {
							chunkSetsInGraph.set(chunksKey, new Set(chunkGraph.getModuleChunksIterable(module)));
						}
					}
